...
class AddToListView: UIView {
    ...
    func setupUI() {
        ...
        let heightBottomView = self.frame.height / 5 - 20
        print("this is height of bottom \(heightBottomView)")
        addSubview(bottomView)
        bottomView.snp.makeConstraints { (make) in
            make.bottom.equalTo(self.safeAreaLayoutGuide).inset(heightBottomView)
            make.width.equalTo(frame.width)
            make.height.equalTo(heightBottomView)
        }
    ...
    let addToList: UIButton = {
        ...
        button.addTarget(self, action: #selector(AddToListController.addToList(_:)), for: .touchUpInside)
        return button
    }()
